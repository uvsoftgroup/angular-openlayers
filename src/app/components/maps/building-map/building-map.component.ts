...
export class BuildingMapComponent implements OnInit {
  ...
  public osmMapDynamicDataView() {
    ...
    */
    ...
    let i, rv, featurePolygonBuilding, coordinatesBuilding, vectorSourceBuilding, vectorLayerBuilding;
    for (i = 1; i <= 10; i++) {
      rv = Math.random();
      coordinatesBuilding = [[
    ...
    function onClick(args) {
      ...
      const lon = lonlat[0];
      const lat = lonlat[1];
      const polyGeom = featurePolygonBuilding.getGeometry().getExtent();
      const center = ol.extent.getCenter(polyGeom);
      // we can get the closest feature from the source
      const feature = vectorLayerBuilding.getSource().getClosestFeatureToCoordinate(polyGeom);
