...
export class PlotMapComponent implements OnInit {
  ...
  public osmMapDynamicDataView() {
    ...
    */
    ...
    let i, rv, featurePolygonPlot, coordinatesPlot, vectorSourcePlot, vectorLayerPlot;
    for (i = 1; i <= 10; i++) {
      rv = Math.random();
      coordinatesPlot = [[
    ...
    function onClick(args) {
      ...
      const lon = lonlat[0];
      const lat = lonlat[1];
      const polyGeom = featurePolygonPlot.getGeometry().getExtent();
      const center = ol.extent.getCenter(polyGeom);
      // we can get the closest feature from the source
      const feature = vectorLayerPlot.getSource().getClosestFeatureToCoordinate(polyGeom);
